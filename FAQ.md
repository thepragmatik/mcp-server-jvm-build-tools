...
Yes. The primary deployment model is a plain JAR: `java -jar mcp-server-jvm-build-tools.jar`. Docker is an alternative for containerized setups. See the [Quick Start](QUICKSTART.md) for both options.
...
Security. The server enforces an allowlist of safe build commands and blocks dangerous flags that could execute arbitrary code. See [SECURITY.md](SECURITY.md) for the full security model.
...
The server is extensible via the `BuildTool` SPI. To add support for Bazel, Ant, Mill, or another build tool, implement the `BuildTool` interface and register it with `BuildToolProvider`. See [ARCHITECTURE.md](ARCHITECTURE.md) for the extension guide.
...
Stdio transport is local — it runs within the same machine between the MCP client and the server process. It does not go over a network. The attack surface is MCP JSON-RPC messages, filesystem paths, and spawned processes. See [SECURITY.md](SECURITY.md) for the full threat model.
...
See [CONTRIBUTING.md](CONTRIBUTING.md) for the full guide. The branch workflow is: feature branch → PR to `staging` → PR to `main`. All PRs require passing CI on JDK 21, 23, and 25.
