...
- **Tool Authorization & Audit**: 4 new tools — `check_tool_authorization` (pre-validate tool access against scope permissions), `list_available_scopes` (enumerate scopes with tool coverage), `audit_tool_access` (read OWASP MCP06 audit logs), `validate_access_token` (validate API keys configured via BUILD_TOOLS_API_KEY_* env vars). Scope-based permission model with wildcard support and audit logging. Implements MCP Server Authorization specification.
...
- **MCP Server Card**: GET /.well-known/mcp-server endpoint for discoverability — metadata, capabilities, transports, features, security posture.
- **Streamable HTTP Transport**: Deploy as a web service with health checks alongside stdio. Container probes at /health/ready and /health/live for Kubernetes and Docker orchestration.
...
- **ROADMAP.md**: Research-driven enhancement plan covering MCP 2026-07-28 protocol migration (P0), async build tasks (P1), and completed features (P2-P4).
- **375 tests** across security, functionality, integration, and MCP protocol compliance across 22 test classes.
...
- Updated README, TOOLS.md, and docs/INDEX.md to reflect full tool set and MCP-004 enhancements.
...
[Unreleased]: https://github.com/thepragmatik/mcp-server-jvm-build-tools/compare/v0.2.0...HEAD
[0.2.0]: https://github.com/thepragmatik/mcp-server-jvm-build-tools/compare/v0.1.0...v0.2.0
